...
def list_blood_glucose(
    ...
    return BloodGlucoseRepository(session).get_all(user_id)
...
def create_blood_glucose(
    ...
    try:
        return BloodGlucoseRepository(session).create(data, user_id)
...
async def import_blood_glucose(
    ...
    try:
        BloodGlucoseRepository(session).bulk_create(items, user_id)
    except DuplicateMeasurementError as exc:
        raise HTTPException(status_code=409, detail=str(exc))
...
def blood_glucose_chart(
    ...
    records = BloodGlucoseRepository(session).get_in_range(
...
def get_blood_glucose(
    ...
    record = BloodGlucoseRepository(session).get(record_id, user_id)
    if not record:
...
def update_blood_glucose(
    record_id: int,
    data: BloodGlucoseUpdate,
    session: Session = Depends(get_session),
    user_id: str = Depends(get_current_user_id),
):
    try:
        record = BloodGlucoseRepository(session).update(record_id, data, user_id)
    except DuplicateMeasurementError as exc:
        raise HTTPException(status_code=409, detail=str(exc))
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
...
def delete_blood_glucose(
    ...
    if not BloodGlucoseRepository(session).delete(record_id, user_id):
        raise HTTPException(status_code=404, detail="Record not found")
