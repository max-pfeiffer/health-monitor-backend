...
- When a pull request is merged to main, the release workflow runs commitizen (`cz bump`) to determine the next version from the conventional commits, update `pyproject.toml` and `CHANGELOG.md`, commit, and push a semver tag back to main
- If no commits since the last tag warrant a bump, the workflow exits without releasing
- A GitHub release is created automatically from the bumped tag, with release notes generated by commitizen from the changelog
- On a successful release, the container image is built and pushed to Docker Hub, tagged with the release version and `latest`
- On a successful release, the OpenAPI specs are regenerated and a PR is opened against main with the updated `api_docs/`
